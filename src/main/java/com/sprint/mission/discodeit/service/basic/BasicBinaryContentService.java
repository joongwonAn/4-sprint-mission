...
public class BasicBinaryContentService implements BinaryContentService {
    ...
    public BinaryContentDto create(BinaryContentCreateRequest request) {
        ...
        log.info("# 바이너리 컨텐츠 생성 완료: id={}, fileName={}, size={}, status={}",
                binaryContent.getId(), fileName, bytes.length, binaryContent.getStatus());
    ...
    public void updateStatus(UUID binaryContentId, BinaryContentStatus status) {
        log.debug("# 바이너리 컨텐츠 상태 업데이트 시작: id={}, newStatus={}", binaryContentId, status);
        ...
        publisher.publishEvent(new FileUploadStatusChangedEvent(binaryContentDto.id()));
    }
}
